...
def fill_template(template_dict, copyright_dict):
    ...
    template_dict["Package Information"][0]["PackageLicenseDeclared"] = copyright_dict[
        "File Information"
    ][0]["LicenseInfoInFile"]
    template_dict["Package Information"][0]["PackageCopyrightText"] = copyright_dict[
        "File Information"
    ][0]["FileCopyrightText"]
    template_dict["Package Information"][0]["PackageLicenseComments"] = [
        copyright_dict["Package Information"][0]["PackageLicenseComments"][0].replace(
            "licenseInfoInFile", "PackageLicenseDeclared"
        )
    ]
    if len(template_dict["Package Information"][0]["PackageLicenseDeclared"]) == 1:
        template_dict["Package Information"][0][
            "PackageLicenseConcluded"
        ] = template_dict["Package Information"][0]["PackageLicenseDeclared"]
    else:
        template_dict["Package Information"][0]["PackageLicenseConcluded"] = [
            "NOASSERTION"
        ]
...
def merge_control(spdx_dict, control_dict):
    ...
    spdx_dict["Package Information"][0]["PackageVersion"] = control_dict[
        "PackageVersion"
    ]
    spdx_dict["Package Information"][0]["SPDXID"] = control_dict["SPDXID"]
    spdx_dict["Package Information"][0]["PackageComment"] = control_dict[
        "PackageComment"
    ]

    spdx_dict["Package Information"][1]["Relationship"] = [
        spdx_dict["Document Information"][0]["SPDXID"][0]
        + " DESCRIBES "
        + spdx_dict["Package Information"][0]["SPDXID"][0]
    ]
