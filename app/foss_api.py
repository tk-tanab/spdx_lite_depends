...
def get_access_token(token_num, token_name, access_token_json):
    ...
    body = {"username": "fossy", "password": "fossy", "token_scope": "write"}
    ...
    result = requests.post(
        url + "/repo/api/v1/tokens", headers=headers, data=json.dumps(body)
    ).json()
...
def take_access_token(access_token_dict, access_token_json):
    ...
    token_expire_date = datetime.datetime.strptime(
        access_token_dict["token_expire"], "%Y-%m-%d"
    ).date()
    today = datetime.date.today()
    ...
    if token_expire_date <= today:
        return get_access_token(
            access_token_dict["token_num"],
            access_token_dict["token_name"],
            access_token_json,
        )
    # 期限が切れてなかった時
    else:
...
def upload_copyright(token, copyright_file, folderID):
    ...
    with open(copyright_file, "rb") as file:
        result = requests.post(
            url + "/repo/api/v1/uploads", headers=headers, files={"fileInput": file}
        ).json()
